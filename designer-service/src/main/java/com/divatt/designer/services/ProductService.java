...
//					RestTemplate followerData= new RestTemplate();
						...
						ResponseEntity<String> forEntity = restTemplate.getForEntity(
								"https://localhost:8082/dev/user/followedUserList/" + productData.getDesignerId(),
								String.class);
						String data = forEntity.getBody();
						JSONArray jsonArray = new JSONArray(data);
						String designerImageData = designerProfileRepo
								.findBydesignerId(productData.getDesignerId().longValue()).get().getDesignerProfile()
								.getProfilePic();
						for (int i = 0; i < jsonArray.length(); i++) {
							ObjectMapper objectMapper = new ObjectMapper();
							UserProfile readValue = objectMapper.readValue(jsonArray.get(i).toString(),
									UserProfile.class);
//						RestTemplate userResponse= new RestTemplate();
							ResponseEntity<UserProfileInfo> userInfo = restTemplate.getForEntity(
									"https://localhost:8082/dev/user/getUserId/" + readValue.getUserId(),
									UserProfileInfo.class);
							userInfoList.add(userInfo.getBody());
						}
						for (int i = 0; i < userId.size(); i++) {
							ResponseEntity<UserProfileInfo> userProfileList = restTemplate.getForEntity(
									"https://localhost:8080/dev/auth/info/USER/" + userId.get(i),
									UserProfileInfo.class);
...
//					Path filePath= Path.of("D:\\packageservice\\projects\\Divatt\\divatt-backend-updated-4-05-2022\\divatt-backend\\designer-service\\src\\main\\resources\\templates\\emailTemplate.html");
						...
						String ETreplace3 = ETreplace2.replace(productPrice,
								productData.getPrice().getIndPrice().getDealPrice().toString());
						String ETreplace4 = ETreplace3.replace(productDiscount,
								productData.getPrice().getIndPrice().getDiscountValue().toString());
						String ETreplace5 = ETreplace4.replace(productLink, "http://65.1.190.195/divatt/product-detail/"
								+ newProductData.getProductId().toString());
						...
						System.out.println("http://65.1.190.195/divatt/product-detail/"
								+ newProductData.getProductId().toString());
...
//						System.out.println(userInfoList.get(i).getEmail());
							EmailSenderThread emailSenderThread = new EmailSenderThread(userInfoList.get(i).getEmail(),
									"New Product Available", ETreplace8, true, null);
