			...
			return !searchKey.equals("") ? productRepo2.findbySearchKey(searchKey)
					: productRepo2.findAll().stream()
							...
							.filter(product -> !maxPrice.equals("-1")
									? product.getMrp() <= Long.parseLong(maxPrice)
									: true)
							.filter(product -> !minPrice.equals("-1")
									? product.getMrp() >= Long.parseLong(minPrice)
									: true)
							.filter(product -> !size.equals("") ? product.getSoh().equals(searchKey): true)
