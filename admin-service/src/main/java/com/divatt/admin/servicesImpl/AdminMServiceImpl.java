...
public class AdminMServiceImpl implements AdminMService {
		...
		try {
			...
			if (t) {
				...
				return new GlobalResponse(MessageConstant.SUCCESS.getMessage(),
						MessageConstant.COLOUR_ADDED.getMessage(), 200);
			}
			return new GlobalResponse(MessageConstant.FAILED.getMessage(),
					MessageConstant.COLOUR_NOT_ADDED.getMessage(), 404);
		} catch (Exception e) {
			throw new CustomException(e.getMessage());
		}
	}
		...
		try {
			...
			for (int i = 0; i < entity.size(); i++) {
				if (entity.get(i).getColorName().equals(name)) {
					...
					return new GlobalResponse(MessageConstant.SUCCESS.getMessage(),
							MessageConstant.COLOUR_UPDATED.getMessage(), 200);
				}
			}
			return new GlobalResponse(MessageConstant.FAILED.getMessage(),
					MessageConstant.COLOUR_NOT_UPDATED.getMessage(), 404);
		} catch (Exception e) {
			throw new CustomException(e.getMessage());
		}
	}
		...
		try {
			...
			for (int i = 0; i < entity.size(); i++) {
				if (entity.get(i).getIsActive().equals(true) && entity.get(i).getColorName().equals(name)) {
					...
					adminMDataRepo.save(coloEntity);
					return new GlobalResponse(MessageConstant.SUCCESS.getMessage(),
							MessageConstant.COLOUR_INACTIVATED.getMessage(), 200);
				}
			}
			return new GlobalResponse(MessageConstant.FAILED.getMessage(),
					MessageConstant.COLOUR_ACTIVATED.getMessage(), 404);
		} catch (Exception e) {
			throw new CustomException(e.getMessage());
		}
	}
			...
			for (int i = 0; i < listColour.size(); i++) {
				if (listColour.get(i).getColorName().contentEquals(name)) {
					if (res) {
						...
						adminMDataRepo.save(colourMetaEntity);
						return new GlobalResponse(MessageConstant.SUCCESS.getMessage(),
								MessageConstant.COLOUR_DELETED.getMessage(), 200);
					}
				}
			}

			return new GlobalResponse(MessageConstant.FAILED.getMessage(),
					MessageConstant.COLOUR_NOT_DELETED.getMessage(), 404);
		} catch (Exception e) {
			throw new CustomException(e.getMessage());
		}
	}
	...
	public GlobalResponse addBanner(BannerEntity bannerEntity) {
		try {
			...
			if (findByBannerName.size() >= 1) {
				...
				return new GlobalResponse(MessageConstant.SUCCESS.getMessage(),
						MessageConstant.BANNER_ADDED.getMessage(), 200);
	...
	public GlobalResponse updateBanners(@Valid BannerEntity bannerEntity, Long id) {
		...
		try {
			...
			if (banEntity.getIsDeleted().equals(false)) {
				...
				return new GlobalResponse(MessageConstant.SUCCESS.getMessage(),
						MessageConstant.BANNER_UPDATED.getMessage(), 200);
			} else {
				return new GlobalResponse(MessageConstant.FAILED_BANNER.getMessage(),
						MessageConstant.BANNER_NOT_UPDATED.getMessage(), 200);
			}
		} catch (Exception e) {
			throw new CustomException(e.getMessage());
		}
	...
	public GlobalResponse deleteBanner(Long id) {
		...
		if (bannerEntity.getIsDeleted().equals(false)) {
			...
			return new GlobalResponse(MessageConstant.SUCCESS.getMessage(), MessageConstant.BANNER_DELETED.getMessage(),
					200);
		} else {
			return new GlobalResponse(MessageConstant.FAILED.getMessage(),
					MessageConstant.BANNER_NOT_DELETED.getMessage(), 404);
		}
	}
		...
		try {
			...
			for (int i = 0; i < designerCategoryEntity.getDesignerLevels().size(); i++) {
				if (!designerEntity.getDesignerLevels().contains(designerCategoryEntity.getDesignerLevels().get(i))) {
					...
					return new GlobalResponse(MessageConstant.SUCCESS.getMessage(),
							MessageConstant.DESIGNER_CATEGORY_ADDED.getMessage(), 200);
				}
			}
			return new GlobalResponse(MessageConstant.FAILED.getMessage(),
					MessageConstant.DESIGNER_CATEGORY_NOT_ADDED.getMessage(), 404);
		} catch (Exception e) {
			throw new CustomException(e.getMessage());
		}
	}
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				if (list.get(i).equals(name)) {
					...
					designerCategoryRepo.save(designerEntity);
					return new GlobalResponse(MessageConstant.SUCCESS.getMessage(),
							MessageConstant.DESIGNER_CATEGORY_UPDATED.getMessage(), 200);
				} else {
					return new GlobalResponse(MessageConstant.FAILED.getMessage(),
							MessageConstant.DESIGNER_CATEGORY_NOT_UPDATED.getMessage(), 404);
				}
			}
			return null;
		} catch (Exception e) {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (list.get(i).equals(name)) {
					...
					designerCategoryRepo.save(designerEntity);
					return new GlobalResponse(MessageConstant.SUCCESS.getMessage(),
							MessageConstant.DESIGNER_LEVEL_DELETED.getMessage(), 200);
				} else {
					return new GlobalResponse(MessageConstant.FAILED.getMessage(),
							MessageConstant.DESIGNER_LEVEL_NOT_EXIST.getMessage(), 404);
				}
			}
			return null;
		} catch (Exception e) {
	...
	public GlobalResponse changeBannerStatus(Long id) {
		...
		if (bannerEntity.getIsDeleted().equals(false) && bannerEntity.getIsActive().equals(true)) {
			...
			bannerRepo.save(bannerEntity);
			return new GlobalResponse(MessageConstant.SUCCESS.getMessage(),
					MessageConstant.BANNER_INACTIVE.getMessage(), 200);
		} else {
			bannerEntity.setIsActive(true);
			bannerRepo.save(bannerEntity);
			return new GlobalResponse(MessageConstant.SUCCESS.getMessage(), MessageConstant.BANNER_ACTIVE.getMessage(),
					200);
		}
